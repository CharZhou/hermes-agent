...
def _dynamic_context_file_max_chars(context_length: Optional[int], *, allow_below_default: bool = False) -> int:
...
def _get_context_file_max_chars(context_length: Optional[int] = None, *, allow_below_default: bool = False) -> int:
    ...
    return int(val) if isinstance(val, (int, float)) and val > 0 else _dynamic_context_file_max_chars(
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None,
                 *, allow_below_default: bool = False) -> Optional[str]:
    ...
    try:
        ...
        return _truncate_content(_scan_context_content(content, "SOUL.md"), "SOUL.md", context_length=context_length,
                                 read_path=str(soul_path), allow_below_default=allow_below_default)
    except Exception as e:
...
def _context_section(content: str, label: str, warn_name: str, path: Path, context_length: Optional[int],
                     *, allow_below_default: bool = False) -> str:
...
def _load_cursorrules(cwd_path: Path, context_length: Optional[int] = None, *, allow_below_default: bool = False) -> str:
    ...
    return _truncate_content(cursorrules_content, ".cursorrules", context_length=context_length,
                             read_path=str(cwd_path / ".cursorrules"), allow_below_default=allow_below_default)
...
def build_context_files_prompt(
    ...
    *, allow_below_default: bool = False,
    ...
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        ...
        sections = [_load_hermes_md(cwd_path, context_length, allow_below_default=allow_below_default)
    ...
    if not skip_soul:
        sections.append(load_soul_md(context_length, home_override=home_override, allow_below_default=allow_below_default))

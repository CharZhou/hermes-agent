...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    if (
        ...
        or not 0 < runtime_ctx < minimum_context
    ):
        return None
    ...
    logger.warning(
        ...
        getattr(agent, "base_url", "") or "unknown base URL", runtime_ctx, minimum_context,
    ...
    return (
        ...
        f"needs at least {minimum_context:,} tokens for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the model before trying "
        f"again. A known-good starting point is {minimum_context:,} tokens. In Hermes config, set "
        f"`model.ollama_num_ctx: {minimum_context}` (and `model.context_length: {minimum_context}` if you also override the "
        "displayed model context). If you manage the model through an Ollama Modelfile, set "
        f"`PARAMETER num_ctx {minimum_context}` there instead."
    )

...
class ToolSearchConfig:
    ...
    def from_raw(
        ...
        if raw is True:
            return cls(
                enabled="on" if small_context_mode else "auto",
        ...
        if raw is False:
            return cls(
                enabled="on" if small_context_mode else "off",
                threshold_pct=2.0 if small_context_mode else 5.0,
                search_default_limit=5,
                max_search_limit=25,
            )
...
def load_config_readonly(*, small_context_mode: bool = False) -> ToolSearchConfig:

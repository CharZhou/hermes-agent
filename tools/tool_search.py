...
class ToolSearchConfig:
    ...
    @classmethod
    def from_raw(cls, raw: Any, *, small_context_mode: bool = False) -> "ToolSearchConfig":
        ...
        return cls(
            enabled="on" if small_context_mode else _tri_state(raw.get("enabled", "auto")),
            threshold_pct=max(0.0, min(2.0 if small_context_mode else 100.0, _safe_float(raw.get("threshold_pct"), 5.0))),
...
def _config_from_loader(loader_name: str, *, small_context_mode: bool = False) -> ToolSearchConfig:
    ...
    try:
        ...
        return ToolSearchConfig.from_raw(tools_cfg.get("tool_search"), small_context_mode=small_context_mode)
